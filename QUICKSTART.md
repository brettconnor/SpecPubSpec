...
The same canonical markdown file is simultaneously:
...
|---|---|---|
| Seed Doc | [`docs/seed-doc.md`](docs/seed-doc.md) | `/` |
...
Do not add markdown content directly under `site/`; edit the canonical source file at its repository path instead (`docs/seed-doc.md`), then rebuild.
...
Any pull request that edits `docs/seed-doc.md` should identify which requirement(s) or section the change affects, per `AGENTS.md`'s Canonical Document & Governance Notes, and should run `scripts/sync-readme.sh` to keep `README.md` in sync.
