...
5. **Review `scripts/audit-repo.sh`'s `EXEMPT_PATHS`**: this reverse-audit script (REQ-044) fails the build if any file or directory on disk isn't either required by `validate-structure.sh` or listed in `audit-repo.sh`'s own `EXEMPT_PATHS` array (currently `.gitignore`, `site/.gitignore`, `LICENSE`, `CODE_OF_CONDUCT.md`, `CONTRIBUTING.md`, `SECURITY.md` — hygiene files out of SpecPubSpec's scope). If you add, remove, or rename repo-hygiene files (e.g. add a `CHANGELOG.md`, `NOTICE`, or `.github/ISSUE_TEMPLATE/`), update `EXEMPT_PATHS` to match, or `governance-check.yml` will fail on files it doesn't recognize.
